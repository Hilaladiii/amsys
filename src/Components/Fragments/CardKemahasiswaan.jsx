...
const CardKemahasiswaan = ({title, logo, description, salary,color}) => {
    return (
        <div className="w-[600px] h-[130px] shadow-md rounded-lg pr-6 duration-300 animate-fade-down animate-delay-300">
                    ...
                    <div className="w-[45vw] flex flex-row justify-between items-start">
                        <div className="flex flex-col items-start gap-2 px-5 py-4 group-hover:text-white">
                            <div className="flex flex-row items-center gap-2">
                                ...
                                <img className="w-[40px] h-[40px]" src={`./images/kemahasiswaan/${logo}.png`} alt={`logo ${title}`}/>                                </div>
                                <h1 className="font-semibold border px-2 rounded-full" style={{background: `rgba(${color})`, color: "#00000"}} >{title}</h1>
                            </div>
                        </div>
